...
class LoginVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let sid = defaults.object(forKey: "sid") as? String {
                            ...
                            if let result = dict["result"] as? String {
                                if (result == "success") {
                                    if let data = dict["data"] as? [[String: Any]] {
                                        if let librarian = data[0]["librarian"] as? Int {
                                            if librarian == 1 {
                                                DispatchQueue.main.async {
                                                    let storyboard = UIStoryboard(name: "Main", bundle: nil)
                                                    let tabBarController = storyboard.instantiateViewController(withIdentifier: "librarianTabViewController") as! UITabBarController
